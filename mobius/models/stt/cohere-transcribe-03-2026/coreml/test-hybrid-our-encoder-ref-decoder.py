#!/usr/bin/env python3
"""Test hybrid: our encoder + BarathwajAnandan's reference decoder."""
...
print("="*70)
print("Testing Hybrid: Our Encoder + Reference Decoder")
print("="*70)
...
try:
    ...
    print(f"   ✓ Reference decoder loaded")
...
print(f"   ✓ Our encoder output: {encoder_hidden.shape}")

# Run REFERENCE decoder with our encoder output
print(f"   Running REFERENCE decoder...")
...
for step in range(max_new_tokens):
    ...
    decoder_output = ref_decoder.predict(decoder_input)

    # Extract outputs (reference decoder output names might differ)
    logits = None
    for key, value in decoder_output.items():
        if hasattr(value, 'shape'):
            if len(value.shape) == 2 and value.shape[1] > 1000:
                ...
                if cache_k.shape == value.shape:
                    if 'k' in key.lower() or 'var_2894' in key:
                        cache_k = value
                    else:
                        cache_v = value
...
print("="*70)
print("\nConclusion:")
if tokens[-1] == eos_token_id and len(tokens) < max_new_tokens:
    print("  ✅ Our encoder works perfectly with reference decoder!")
    print("  ✅ Issue is 100% in our decoder export")
else:
    print("  ⚠️  Unexpected behavior - may indicate encoder issue")
print()
